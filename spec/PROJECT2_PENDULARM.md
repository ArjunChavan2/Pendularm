...
Reuses the [rosbridge TCP/JSON protocol](../spec/ROSBRIDGE_PROTOCOL.md) — the same external
...
[C++](layouts/cpp.md), and [Rust](layouts/rust.md) — see `starter/`. They are not
partial dynamics/integrator/PID solutions; another layout is allowed.

This handout, the API specs it links to, and the four starter layouts are also available
pre-packaged as the **student kit** — a bundle with no other course-repo material (no reference
solutions, grader, or instructor tooling), built via `project2/scripts/make_student_kit.sh` and
downloadable from [this Drive folder](https://drive.google.com/drive/folders/1EztLZppnoQj80l8m1FI3ZWq5VN97203L?usp=drive_link).
...
concept here. If you have the student kit, `make_submission.sh <c|cpp|python|rust>` at its root
builds this correctly from your completed starter directory; see the kit's `README.md`.
